...
class IntervalTimeSpecification(TimeSpecification):
  ...
  weekdays - restrict the days of the week that this should run, as a set of
             integers, 0=Sunday, 6=Saturday. None means no restriction.
  ...
  def __init__(
    ...
    if self.weekdays is not None:
      if self.weekdays and (min(self.weekdays) < 0 or max(self.weekdays) > 6):
        raise ValueError(
            'weekdays must be between 0 (sun) and 6 (sat) inclusive, got %r'
            % self.weekdays
        )
      if not self.weekdays or len(self.weekdays) == 7:
        # There is no need to do any check for the "all week days" restriction.
        self.weekdays = None
    if self.weekdays is not None and self.seconds > 24 * 3600:
      raise groc.GrocException(
          'weeksdays restriction can only be used with periods that are no'
          ' longer than one day'
      )
  ...
  def GetMatch(self, start):
    ...
    """
    while True:
      candidate = self._GetMatchCandidate(start)
      if self._IsMatchingDayOfWeek(candidate):
        return candidate
      start = start + datetime.timedelta(seconds=self.seconds)

  def _GetMatchCandidate(self, start):
    """Returns the next match after 'start' without the day of week restriction."""
  ...
  def _IsMatchingDayOfWeek(self, t):
    """Checks whether 't' matches the day of week restriction.

    Arguments:
      t: a datetime to check.

    Returns:
      True if the day of week matches or not restricted.
    """
    if self.weekdays is None:
      return True
    t_in_local = _ToTimeZone(t, self.timezone)
    # Groc has sunday = 0, calendar module has monday = 0.
    groc_day_of_week = (t_in_local.weekday() + 1) % 7
    return groc_day_of_week in self.weekdays
  ...
  def __init__(
    ...
    if months is None:
      ...
      if self.months and (min(self.months) < 1 or max(self.months) > 12):
        raise ValueError(
            'months must be between 1 (jan) and 12 (dec) inclusive, got %r'
            % months
        )
    ...
    if not monthdays:
      ...
      if self.months:
        for month in self.months:
          ...
          raise ValueError(
              'invalid day of month, got day %r of month %r'
              % (max(monthdays), month)
          )
